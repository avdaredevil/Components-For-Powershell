...
if (AP-Require "dep:nw" -PassThru) {
    ...
    function Global:NW-Package-App($Dir=".",$BinaryName = "app") {
        ...
        if (!(AP-Require "function:Get-Where" -PassThru)) {Write-AP $SIG,"n!Cannot finalize binary without [Get-Where]";return}
        if (!(AP-Require "dep:7z" -PassThru)) {Write-AP $SIG,"n!Cannot compress app without 7z";return}
        pushd $Dir
        if (!(Test-Path -type leaf "package.json")) {popd;Write-AP $SIG,"nx!Required File [","nx+package.json","n!] is missing";return}
        $JSON = cat .\package.json | ConvertFrom-Json
        Write-AP "x*Creating nw binary for ","nx!$($JSON.name)","nx* [","nx+$($JSON.version)","n*]:"
        $PKG = @{file="app-package.nw";path=$(Split-Path $JSON.main);other=$JSON.include_dirs}
        if ($PKG.other -and $PKG.other.length) {Write-AP "x>*Including Folder: ",(($PKG.other | % {"nx!$_"}) -join "|nx*, |").split("|"),"n*."}
        rm $PKG.file -ea SilentlyContinue
        if (!$PKG.path) {Write-AP ">!No package path defined...";return}
        if (!(test-path -type container $PKG.path)) {Write-AP "x>!Could not access package path [","nx+$($PKG.path)","n!]";return}
        ...
        rm $PKG.file -ea SilentlyContinue
        Write-AP ">+Created Binary - $BinaryName.exe"
        popd
    }
}
if (!$Silent) {Write-AP $Out}
rv PYD
